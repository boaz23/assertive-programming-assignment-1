...
function R_k(a: nat, b: nat, k: int): nat
	requires a > 0
	requires -2 <= k <= N(a, b)
	decreases b
{
	...
	ensures GreatestCommonDivisor(a, b, i)
{
		...
		decreases r_k_m1
	{
		...
		assert k + 1 <= N + 1;
		assert r_k_m2 == R_k(a, b, (k + 1) - 2);
		assert r_k_m1 == R_k(a, b, (k + 1) - 1);

		k := k + 1;
		
		assert k <= N + 1;
		assert r_k_m2 == R_k(a, b, k - 2);
		assert r_k_m1 == R_k(a, b, k - 1);
	}
